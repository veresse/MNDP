...
def pack(atoms, adjs,  labels, smi_ids, prot_ids, device):
    ...
    for atom in atoms: #atom(27,34)
        if atom.shape[0] >= atoms_len:
            atoms_len = atom.shape[0]

    if atoms_len>hp.MAX_DRUG_LEN: atoms_len = hp.MAX_DRUG_LEN
    atoms_new = torch.zeros((N,atoms_len,34), device=device) #(8,49,34)
    i = 0
    for atom in atoms:
        a_len = atom.shape[0]
...
if __name__ == "__main__":
    ...
    for fold, (train_idx, test_idx) in enumerate(skf.split(dataset, labels)):
        ...
        model = MNDT(hp.hid_dim, hp.dropout, device)


        model.to(device)
        trainer = Trainer(model, hp.lr, hp.weight_decay, hp.batch, len(dataset_train), hp.iteration)
        tester = T_ester(model)
